...
//!
//! Think of it as Display without requiring any specific data, which is reflected in the type signature:
//! ```notest
//! fn label() -> &'static str {
//! ```
//!
...
//! }
//!
//! // we will parse these types from strings provided by a 3rd party
...
//!
//! // one error for parsing, generic over what is being parsed
//! #[derive(Debug)]
...
//! // how to impl Display!?
//! ```notest
//! impl Display for ParseError<T> {
//!     fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
//!         write!(f, "there was an error parsing... T :(")
//!     }
//! ```
//! we could make T: Display, but if we *haven't parsed a T* how can we rely on T's implementation of... anything?
//!
//! // enter type_label
//! ```
//! # use std::error::Error;
...
//! }
//! 
//! // provide a new() method to make avoid dealing with PhantomData at call site
...
//! }
//!
//! // we let rust know our type is an Error
//! impl<T: Label> Error for ParseError<T> {}
//!
//! // we give our types labels
//!
//! #[derive(Label)]
//! #[label = "activity type"]
//! pub enum ActivityType {
...
//! }
//!
//! // we return ParseError when something goes wrong parsing our type
...
//! but would implement `Label` to display to the user itself as a concept: "type of Animal".
//!
